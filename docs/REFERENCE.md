...
## Dashboard and refresh behavior

- Connect to one or more CLIProxyAPI services, each with its own management URL and key, and switch between them instantly from the dashboard title's dropdown (see [Multiple Services](#multiple-services)).
...
## Storage and notification behavior
