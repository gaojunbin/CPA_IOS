...
func runValidation() async throws {
    ...
    try expect(appStoreMetadata.contains("notification-tap dashboard view"), "App Store metadata should include notification tap screenshot planning")
    ...
    try expect(appStoreMetadata.contains("dashboard server hosts, dashboard account identifiers, project IDs"), "App Store metadata should document dashboard privacy-sensitive fields")
    ...
    try expect(supportPage.contains("Tapping a low-quota alert opens the dashboard"), "support page should document notification tap routing")
    ...
    try expect(privacyPolicy.contains("opens the app to the dashboard"), "privacy policy should document notification tap routing")
    ...
    try expect(privacyPolicy.contains("dashboard server hosts, dashboard account identifiers, project IDs"), "privacy policy should document dashboard privacy-sensitive fields")
    ...
    try expect(readme.contains("Sort local alert candidates with that same threshold"), "README should document shared alert sorting threshold")
